...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-labels_file", "--labels_file", type=str, help="Labels file directory (str)"
    )
    parser.add_argument(
        "-fs", "--fs", type=int, help="Sample rate in Hz (int). Default: 16000"
    )
    parser.add_argument(
        "-o",
        "--o",
        type=str,
        help="Output directory (str). Default: root_directory_folder/dataset",
    )
    parser.add_argument(
        "-multiprocessing",
        "--multiprocessing",
        ...
        help="Use multiprocessing (bool). Default: False",
    )
    parser.add_argument(
        "-overwrite",
        "--overwrite",
        action="store_true",
        dest="overwrite",
        help="Overwrite existing files (bool). Default: False",
    )
    parser.add_argument(
        "-c",
        "--c",
        nargs="+",
        type=str.lower,
        help="Specify the classes you want to download (str). Default: all",
    )
    parser.add_argument(
        "-max_files",
        "--max_files",
        type=int,
        help="Max files to download from each class (int). Default: None (all of them)",
    )
    parser.add_argument(
        "-channels",
        "--channels",
        type=int,
        help="Channels audio output (int). Default: 1",
    )
    parser.set_defaults(
        fs=16000,
    ...
    assert (args.max_files == None) or (
        (type(args.max_files) == int) and (args.max_files > 0)
    ), f"You must provide a valid value for max_files or let it empty to download everything"

    ## mapping the chosen classes
    if args.c != "all":
        assert all(
            [c in all_classes for c in args.c]
        ), f"The class(es) must be {all_classes}"
    ...
    if args.labels_file == None:
        ...
        assert os.path.exists(
            os.path.join(args.labels_file, "ava_speech_labels_v1.csv")
        ), f"You need to download the ava_speech_labels_v1 csv file or keep the labels_file param None"
    ...
    with open("ava_speech_file_names_v1.txt", "r") as f:
        ...
        for line in lines:
            ...
            file_names_df = pd.concat(
                [
                    file_names_df,
                    pd.DataFrame({"file_name": [file_name], "extension": [extension]}),
                ],
                axis=0,
            )
